...
[ExportCodeFixProvider(LanguageNames.CSharp, Name = nameof(SecuritySecretCodeFix))]
[Shared]
public sealed class SecuritySecretCodeFix : CodeFixProvider
{
    public override ImmutableArray<string> FixableDiagnosticIds =>
        ImmutableArray.Create(DiagnosticIds.SecurityHardcodedSecret);
    ...
    private static async Task<Document> ApplyFixAsync(Document document, Diagnostic diagnostic, CancellationToken cancellationToken)
    {
        ...
        var fieldNode = parent.AncestorsAndSelf().OfType<FieldDeclarationSyntax>().FirstOrDefault();
        if (fieldNode is null)
        {
            return document;
        }

        if (!HasSuppressMessage(fieldNode, diagnostic.Id))
        {
            var suppressionAttribute = BuildSuppressionAttribute(diagnostic.Id);
            root = root.ReplaceNode(fieldNode, fieldNode.AddAttributeLists(suppressionAttribute));
        }
    ...
    private static AttributeListSyntax BuildSuppressionAttribute(string diagnosticId)
    {
        var categoryArgument = SyntaxFactory.AttributeArgument(
            SyntaxFactory.LiteralExpression(SyntaxKind.StringLiteralExpression, SyntaxFactory.Literal("Security")));
        ...
        var justificationArgument = SyntaxFactory.AttributeArgument(
            ...
            SyntaxFactory.LiteralExpression(
                ...
                SyntaxFactory.Literal("Secret is loaded from secure configuration or environment at runtime.")));
    ...
    private static bool HasSuppressMessage(FieldDeclarationSyntax fieldNode, string diagnosticId)
    {
        return fieldNode.AttributeLists
