...
namespace Vc.CodeFixes.Resilience;

[ExportCodeFixProvider(LanguageNames.CSharp, Name = nameof(ResilienceCancellationCodeFix))]
[Shared]
public sealed class ResilienceCancellationCodeFix : CodeFixProvider
{
    public override ImmutableArray<string> FixableDiagnosticIds =>
        ImmutableArray.Create(DiagnosticIds.ResilienceCancellationTokenMissing);
    ...
    private static AttributeListSyntax BuildSuppressionAttribute(string diagnosticId)
    {
        ...
        {
            SyntaxFactory.AttributeArgument(SyntaxFactory.LiteralExpression(SyntaxKind.StringLiteralExpression, SyntaxFactory.Literal("Resilience"))),
            ...
            SyntaxFactory.AttributeArgument(SyntaxFactory.NameEquals("Justification"), null,
                SyntaxFactory.LiteralExpression(SyntaxKind.StringLiteralExpression,
                    SyntaxFactory.Literal("Cancellation is controlled by the caller or operation is intentionally non-cancellable.")))
        });
