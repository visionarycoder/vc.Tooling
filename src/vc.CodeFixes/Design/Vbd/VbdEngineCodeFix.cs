...
[ExportCodeFixProvider(LanguageNames.CSharp, Name = nameof(VbdEngineCodeFix))]
[Shared]
public sealed class VbdEngineCodeFix : CodeFixProvider
{
    public override ImmutableArray<string> FixableDiagnosticIds =>
        ImmutableArray.Create(DiagnosticIds.VbdEngineVaultInfrastructureAccess);
    ...
    private static async Task<Document> ApplyFixAsync(Document document, Diagnostic diagnostic, CancellationToken cancellationToken)
    {
        ...
        var typeDeclaration = parent.AncestorsAndSelf().OfType<ClassDeclarationSyntax>().FirstOrDefault();
        if (typeDeclaration is null)
        {
            return document;
        }

        if (!HasSuppressMessage(typeDeclaration, diagnostic.Id))
        {
            var suppressionAttribute = BuildSuppressionAttribute(diagnostic.Id);
            root = root.ReplaceNode(typeDeclaration, typeDeclaration.AddAttributeLists(suppressionAttribute));
        }
    ...
    private static AttributeListSyntax BuildSuppressionAttribute(string diagnosticId)
    {
        ...
        var justificationArgument = SyntaxFactory.AttributeArgument(
            ...
            SyntaxFactory.LiteralExpression(
                ...
                SyntaxFactory.Literal("Reviewed manually; planned infrastructure abstraction refactor.")));
    ...
    private static bool HasSuppressMessage(ClassDeclarationSyntax declaration, string diagnosticId)
    {
