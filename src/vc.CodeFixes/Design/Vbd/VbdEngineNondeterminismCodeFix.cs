...
[ExportCodeFixProvider(LanguageNames.CSharp, Name = nameof(VbdEngineNondeterminismCodeFix))]
[Shared]
public sealed class VbdEngineNondeterminismCodeFix : CodeFixProvider
{
    public override ImmutableArray<string> FixableDiagnosticIds =>
        ImmutableArray.Create(DiagnosticIds.VbdEngineVaultNondeterminism);
    ...
    private static async Task<Document> ApplyFixAsync(Document document, Diagnostic diagnostic, CancellationToken cancellationToken)
    {
        ...
        if (!HasSuppressMessage(method, diagnostic.Id))
        {
            var suppressionAttribute = BuildSuppressionAttribute(diagnostic.Id);
            root = root.ReplaceNode(method, method.AddAttributeLists(suppressionAttribute));
        }
    ...
    private static AttributeListSyntax BuildSuppressionAttribute(string diagnosticId)
    {
        ...
        var justificationArgument = SyntaxFactory.AttributeArgument(
            ...
            SyntaxFactory.LiteralExpression(
                ...
                SyntaxFactory.Literal("Reviewed manually; deterministic source will replace random/clock/guid usage.")));
    ...
    private static bool HasSuppressMessage(MethodDeclarationSyntax method, string diagnosticId)
    {
