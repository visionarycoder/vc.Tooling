...
namespace Vc.CodeFixes.Observability;

[ExportCodeFixProvider(LanguageNames.CSharp, Name = nameof(ObservabilityTelemetryCodeFix))]
[Shared]
public sealed class ObservabilityTelemetryCodeFix : CodeFixProvider
{
    public override ImmutableArray<string> FixableDiagnosticIds =>
        ImmutableArray.Create(DiagnosticIds.ObservabilityTelemetryMissing);
    ...
    private static AttributeListSyntax BuildSuppressionAttribute(string diagnosticId)
    {
        var categoryArgument = SyntaxFactory.AttributeArgument(
            SyntaxFactory.LiteralExpression(SyntaxKind.StringLiteralExpression, SyntaxFactory.Literal("Observability")));
        ...
        var justificationArgument = SyntaxFactory.AttributeArgument(
            ...
            SyntaxFactory.LiteralExpression(
                ...
                SyntaxFactory.Literal("Telemetry is handled through interceptors or external instrumentation.")));
    ...
    private static bool HasSuppressMessage(MethodDeclarationSyntax methodNode, string diagnosticId)
    {
        return methodNode.AttributeLists
