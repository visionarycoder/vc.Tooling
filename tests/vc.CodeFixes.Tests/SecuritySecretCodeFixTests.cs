...
public sealed class SecuritySecretCodeFixTests
{
    ...
    public async Task RegisterCodeFixesAsync_ShouldAddSuppressMessageAttributeForHardcodedSecret()
    {
        ...
        var fieldNode = root!
            ...
            .First(predicate: node => node.Declaration.Variables.Any(predicate: v => v.Identifier.Text == "ApiKey"));

        var diagnostic = Diagnostic.Create(
            descriptor: new DiagnosticDescriptor(
                id: DiagnosticIds.SecurityHardcodedSecret,
                title: "Hardcoded secret detected",
                messageFormat: "Field contains hardcoded secret value.",
                category: "Security",
        ...
        Assert.Contains(expectedSubstring: DiagnosticIds.SecurityHardcodedSecret, actualString: updatedText.ToString());
    }
    ...
    private static async Task<Document> CreateDocumentAsync(string source)
    {
        ...
        return workspace.AddDocument(projectId: project.Id, name: "SecuritySecret.cs", text: SourceText.From(text: source));
    }
}
