...
public sealed class ResilienceCancellationCodeFixTests
{
    [Fact]
    public async Task RegisterCodeFixesAsync_ShouldAddSuppressMessageForMissingCancellationToken()
    {
        var source = """
            namespace SampleApp;
            public sealed class DataService
            {
                public void LoadRecords(int pageSize) { }
            }
            """;
        ...
        var diagnostic = Diagnostic.Create(
            descriptor: new DiagnosticDescriptor(id: DiagnosticIds.ResilienceCancellationTokenMissing, title: "Missing cancellation token",
                messageFormat: "External call '{0}' does not propagate a CancellationToken.", category: "Resilience",
                defaultSeverity: DiagnosticSeverity.Warning, isEnabledByDefault: true),
            location: methodNode.Identifier.GetLocation(), messageArgs: "LoadRecords");

        var provider = new ResilienceCancellationCodeFix();
        ...
        Assert.Contains(expectedSubstring: DiagnosticIds.ResilienceCancellationTokenMissing, actualString: updatedText.ToString());
    }
