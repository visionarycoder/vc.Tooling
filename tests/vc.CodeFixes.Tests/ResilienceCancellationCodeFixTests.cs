...
public sealed class ResilienceCancellationCodeFixTests
{
    ...
    public async Task RegisterCodeFixesAsync_ShouldAddSuppressMessageForMissingCancellationToken()
    {
        ...
        var diagnostic = Diagnostic.Create(
            new DiagnosticDescriptor(DiagnosticIds.ResilienceCancellationTokenMissing, "Missing cancellation token",
                "External call '{0}' does not propagate a CancellationToken.", "Resilience",
                DiagnosticSeverity.Warning, isEnabledByDefault: true),
            methodNode.Identifier.GetLocation(), "LoadRecords");
        ...
        Assert.Contains(DiagnosticIds.ResilienceCancellationTokenMissing, updatedText.ToString());
    }
