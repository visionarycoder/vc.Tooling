...
public sealed class AsyncVoidCodeFixTests
{
    ...
    public async Task RegisterCodeFixesAsync_ShouldAddSuppressMessageAttributeForAsyncVoid()
    {
        ...
        var methodNode = root!
            ...
            .First(node => node.Identifier.Text == "OnButtonClick");

        var diagnostic = Diagnostic.Create(
            new DiagnosticDescriptor(
                DiagnosticIds.AsyncVoidMethod,
                "Async void method",
                "Method '{0}' returns void but is async.",
                "Async",
        ...
        Assert.Contains(DiagnosticIds.AsyncVoidMethod, updatedText.ToString());
    }
    ...
    private static async Task<Document> CreateDocumentAsync(string source)
    {
        ...
        return workspace.AddDocument(project.Id, "AsyncVoid.cs", SourceText.From(source));
    }
}
