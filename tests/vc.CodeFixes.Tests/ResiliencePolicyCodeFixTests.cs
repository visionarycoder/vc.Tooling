...
public sealed class ResiliencePolicyCodeFixTests
{
    ...
    public async Task RegisterCodeFixesAsync_ShouldAddSuppressMessageForMissingPolicy()
    {
        ...
        var diagnostic = Diagnostic.Create(
            new DiagnosticDescriptor(DiagnosticIds.ResiliencePolicyMissing, "Missing resilience policy",
                "External call '{0}' is not wrapped in any resilience policy.", "Resilience",
                DiagnosticSeverity.Warning, isEnabledByDefault: true),
            methodNode.Identifier.GetLocation(), "ProcessOrder");
        ...
        Assert.Contains(DiagnosticIds.ResiliencePolicyMissing, updatedText.ToString());
    }
