...
public sealed class ResiliencePolicyCodeFixTests
{
    [Fact]
    public async Task RegisterCodeFixesAsync_ShouldAddSuppressMessageForMissingPolicy()
    {
        var source = """
            namespace SampleApp;
        ...
        var diagnostic = Diagnostic.Create(
            descriptor: new DiagnosticDescriptor(id: DiagnosticIds.ResiliencePolicyMissing, title: "Missing resilience policy",
                messageFormat: "External call '{0}' is not wrapped in any resilience policy.", category: "Resilience",
                defaultSeverity: DiagnosticSeverity.Warning, isEnabledByDefault: true),
            location: methodNode.Identifier.GetLocation(), messageArgs: "ProcessOrder");

        var provider = new ResiliencePolicyCodeFix();
        ...
        Assert.Contains(expectedSubstring: DiagnosticIds.ResiliencePolicyMissing, actualString: updatedText.ToString());
    }
