...
public sealed class ResilienceRetryCodeFixTests
{
    ...
    public async Task RegisterCodeFixesAsync_ShouldAddSuppressMessageForMissingRetry()
    {
        ...
        var diagnostic = Diagnostic.Create(
            new DiagnosticDescriptor(DiagnosticIds.ResilienceRetryPolicyMissing, "Missing retry policy",
                "External call '{0}' is not wrapped in a retry policy.", "Resilience",
                DiagnosticSeverity.Warning, isEnabledByDefault: true),
            methodNode.Identifier.GetLocation(), "ChargeCard");
        ...
        Assert.Contains(DiagnosticIds.ResilienceRetryPolicyMissing, updatedText.ToString());
    }
