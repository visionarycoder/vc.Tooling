...
public sealed class PerformanceLinqCodeFixTests
{
    ...
    public async Task RegisterCodeFixesAsync_ShouldAddSuppressMessageForLinqInHotPath()
    {
        ...
        var diagnostic = Diagnostic.Create(
            descriptor: new DiagnosticDescriptor(id: DiagnosticIds.PerfLinqInHotPath, title: "LINQ in hot path",
                messageFormat: "LINQ call '{0}' in performance-critical code.", category: "Performance",
                defaultSeverity: DiagnosticSeverity.Info, isEnabledByDefault: true),
            location: methodNode.Identifier.GetLocation(), messageArgs: "Count");
        ...
        Assert.Contains(expectedSubstring: DiagnosticIds.PerfLinqInHotPath, actualString: updatedText.ToString());
    }
