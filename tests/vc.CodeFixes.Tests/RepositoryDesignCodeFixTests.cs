...
public sealed class RepositoryDesignCodeFixTests
{
    [Fact]
    public async Task RegisterCodeFixesAsync_ShouldAddSuppressMessageAttributeForRepositoryMissing()
    {
        var source = """
        ...
        var classNode = root!
            ...
            .First(predicate: node => node.Identifier.Text == "UserAccessor");

        var diagnostic = Diagnostic.Create(
            descriptor: new DiagnosticDescriptor(
                id: DiagnosticIds.DataDesignRepositoryMissing,
                title: "Repository missing",
                messageFormat: "Type '{0}' should implement repository pattern.",
                category: "Data",
                defaultSeverity: DiagnosticSeverity.Warning,
                isEnabledByDefault: true),
            location: classNode.Identifier.GetLocation(),
            messageArgs: "UserAccessor");

        var provider = new RepositoryDesignCodeFix();
        ...
        Assert.Contains(expectedSubstring: DiagnosticIds.DataDesignRepositoryMissing, actualString: updatedText.ToString());
    }
    ...
    private static async Task<Document> CreateDocumentAsync(string source)
    {
        ...
        return workspace.AddDocument(projectId: project.Id, name: "RepositoryDesign.cs", text: SourceText.From(text: source));
    }
}
