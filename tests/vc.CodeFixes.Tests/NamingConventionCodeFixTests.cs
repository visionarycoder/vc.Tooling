...
public sealed class NamingConventionCodeFixTests
{
    ...
    public async Task RegisterCodeFixesAsync_ShouldAddSuppressMessageAttributeForNamingViolation()
    {
        ...
        var fieldNode = root!
            ...
            .First(node => node.Declaration.Variables.Any(v => v.Identifier.Text == "MY_CONSTANT"));

        var diagnostic = Diagnostic.Create(
            new DiagnosticDescriptor(
                DiagnosticIds.NamingConventionViolation,
                "Naming convention violation",
                "Field name '{0}' violates naming convention.",
                "Naming",
        ...
        Assert.Contains(DiagnosticIds.NamingConventionViolation, updatedText.ToString());
    }
    ...
    private static async Task<Document> CreateDocumentAsync(string source)
    {
        ...
        return workspace.AddDocument(project.Id, "NamingConvention.cs", SourceText.From(source));
    }
}
