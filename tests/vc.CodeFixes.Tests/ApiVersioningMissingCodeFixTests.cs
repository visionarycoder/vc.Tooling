...
public sealed class ApiVersioningMissingCodeFixTests
{
    [Fact]
    public async Task RegisterCodeFixesAsync_ShouldAddSuppressMessageAttributeForVersioningMissing()
    {
        var source = """
        ...
        var classNode = root!
            ...
            .First(predicate: node => node.Identifier.Text == "ProductEndpoint");

        var diagnostic = Diagnostic.Create(
            descriptor: new DiagnosticDescriptor(
                id: DiagnosticIds.ApiVersioningMissing,
                title: "API versioning missing",
                messageFormat: "Endpoint '{0}' does not include version information.",
                category: "Api",
                defaultSeverity: DiagnosticSeverity.Warning,
                isEnabledByDefault: true),
            location: classNode.Identifier.GetLocation(),
            messageArgs: "ProductEndpoint");

        var provider = new ApiVersioningMissingCodeFix();
        ...
        Assert.Contains(expectedSubstring: DiagnosticIds.ApiVersioningMissing, actualString: updatedText.ToString());
    }
    ...
    private static async Task<Document> CreateDocumentAsync(string source)
    {
        ...
        return workspace.AddDocument(projectId: project.Id, name: "ApiVersioningMissing.cs", text: SourceText.From(text: source));
    }
}
