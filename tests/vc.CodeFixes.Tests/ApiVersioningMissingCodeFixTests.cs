...
public sealed class ApiVersioningMissingCodeFixTests
{
    ...
    public async Task RegisterCodeFixesAsync_ShouldAddSuppressMessageAttributeForVersioningMissing()
    {
        ...
        var classNode = root!
            ...
            .First(node => node.Identifier.Text == "ProductEndpoint");

        var diagnostic = Diagnostic.Create(
            new DiagnosticDescriptor(
                DiagnosticIds.ApiVersioningMissing,
                "API versioning missing",
                "Endpoint '{0}' does not include version information.",
                "Api",
        ...
        Assert.Contains(DiagnosticIds.ApiVersioningMissing, updatedText.ToString());
    }
    ...
    private static async Task<Document> CreateDocumentAsync(string source)
    {
        ...
        return workspace.AddDocument(project.Id, "ApiVersioningMissing.cs", SourceText.From(source));
    }
}
