...
public sealed class ResilienceCircuitBreakerCodeFixTests
{
    ...
    public async Task RegisterCodeFixesAsync_ShouldAddSuppressMessageForMissingCircuitBreaker()
    {
        ...
        var diagnostic = Diagnostic.Create(
            new DiagnosticDescriptor(DiagnosticIds.ResilienceCircuitBreakerMissing, "Missing circuit breaker",
                "External call '{0}' is not wrapped in a circuit breaker.", "Resilience",
                DiagnosticSeverity.Warning, isEnabledByDefault: true),
            methodNode.Identifier.GetLocation(), "SendMessage");
        ...
        Assert.Contains(DiagnosticIds.ResilienceCircuitBreakerMissing, updatedText.ToString());
    }
