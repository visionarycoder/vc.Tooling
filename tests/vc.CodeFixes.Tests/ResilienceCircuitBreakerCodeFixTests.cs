...
public sealed class ResilienceCircuitBreakerCodeFixTests
{
    [Fact]
    public async Task RegisterCodeFixesAsync_ShouldAddSuppressMessageForMissingCircuitBreaker()
    {
        var source = """
            namespace SampleApp;
            public sealed class ExternalGateway
            {
        ...
        var diagnostic = Diagnostic.Create(
            descriptor: new DiagnosticDescriptor(id: DiagnosticIds.ResilienceCircuitBreakerMissing, title: "Missing circuit breaker",
                messageFormat: "External call '{0}' is not wrapped in a circuit breaker.", category: "Resilience",
                defaultSeverity: DiagnosticSeverity.Warning, isEnabledByDefault: true),
            location: methodNode.Identifier.GetLocation(), messageArgs: "SendMessage");

        var provider = new ResilienceCircuitBreakerCodeFix();
        ...
        Assert.Contains(expectedSubstring: DiagnosticIds.ResilienceCircuitBreakerMissing, actualString: updatedText.ToString());
    }
