...
public sealed class RepositoryAsyncCodeFixTests
{
    [Fact]
    public async Task RegisterCodeFixesAsync_ShouldAddSuppressMessageAttributeForMissingAsync()
    {
        var source = """
        ...
        var diagnostic = Diagnostic.Create(
            ...
            messageArgs: "GetCount");

        var provider = new RepositoryAsyncCodeFix();
        ...
        Assert.Contains(expectedSubstring: DiagnosticIds.DistributedRepositoryAsyncMissing, actualString: updatedText.ToString());
    }
    ...
    private static async Task<Document> CreateDocumentAsync(string source)
    {
        ...
        return workspace.AddDocument(projectId: project.Id, name: "RepositoryAsync.cs", text: SourceText.From(text: source));
    }
}