...
public sealed class ApiControllerMissingCodeFixTests
{
    ...
    public async Task RegisterCodeFixesAsync_ShouldAddSuppressMessageAttributeForControllerMissing()
    {
        ...
        var classNode = root!
            ...
            .First(node => node.Identifier.Text == "ProductService");

        var diagnostic = Diagnostic.Create(
            new DiagnosticDescriptor(
                DiagnosticIds.ApiDesignControllerMissing,
                "Controller missing",
                "API endpoint '{0}' does not have controller attribute.",
                "Api",
        ...
        Assert.Contains(DiagnosticIds.ApiDesignControllerMissing, updatedText.ToString());
    }
    ...
    private static async Task<Document> CreateDocumentAsync(string source)
    {
        ...
        return workspace.AddDocument(project.Id, "ApiControllerMissing.cs", SourceText.From(source));
    }
}
