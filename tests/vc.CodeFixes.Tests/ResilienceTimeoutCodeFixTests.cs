...
public sealed class ResilienceTimeoutCodeFixTests
{
    [Fact]
    public async Task RegisterCodeFixesAsync_ShouldAddSuppressMessageForMissingTimeout()
    {
        var source = """
            namespace SampleApp;
        ...
        var diagnostic = Diagnostic.Create(
            descriptor: new DiagnosticDescriptor(id: DiagnosticIds.ResilienceTimeoutMissing, title: "Missing timeout",
                messageFormat: "External call '{0}' is not wrapped in a timeout.", category: "Resilience",
                defaultSeverity: DiagnosticSeverity.Warning, isEnabledByDefault: true),
            location: methodNode.Identifier.GetLocation(), messageArgs: "FetchData");

        var provider = new ResilienceTimeoutCodeFix();
        ...
        Assert.Contains(expectedSubstring: DiagnosticIds.ResilienceTimeoutMissing, actualString: updatedText.ToString());
    }
