...
public sealed class MappingAnalyzerTests
{
    [Fact]
    public void SupportedDiagnostics_ShouldContainMappingRule()
    {
        var analyzer = new MappingAnalyzer();
        Assert.Contains(collection: analyzer.SupportedDiagnostics, filter: d => d.Id == DiagnosticIds.MappingUnmappedProperties);
    }
    ...
    private static async Task<ImmutableArray<Diagnostic>> GetDiagnosticsAsync(string source)
    {
        ...
        return await compilation.WithAnalyzers(analyzers: [new MappingAnalyzer()]).GetAnalyzerDiagnosticsAsync();
    }
}
