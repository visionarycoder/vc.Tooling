...
public sealed class ResilienceAnalyzerTests
{
    [Fact]
    public void SupportedDiagnostics_ShouldContainPolicyRule()
    {
        var analyzer = new ResilienceAnalyzer();
        Assert.Contains(collection: analyzer.SupportedDiagnostics, filter: d => d.Id == DiagnosticIds.ResiliencePolicyMissing);
    }
    ...
    private static async Task<ImmutableArray<Diagnostic>> GetDiagnosticsAsync(string source)
    {
        ...
        return await compilation.WithAnalyzers(analyzers: [new ResilienceAnalyzer()]).GetAnalyzerDiagnosticsAsync();
    }
}
