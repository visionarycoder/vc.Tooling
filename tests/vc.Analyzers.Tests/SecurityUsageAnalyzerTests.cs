...
public sealed class SecurityUsageAnalyzerTests
{
    [Fact]
    public void SupportedDiagnostics_ShouldContainInputValidationRule()
    {
        var analyzer = new SecurityUsageAnalyzer();
        Assert.Contains(collection: analyzer.SupportedDiagnostics, filter: d => d.Id == DiagnosticIds.SecurityInputValidationMissing);
    }
    ...
    private static async Task<ImmutableArray<Diagnostic>> GetDiagnosticsAsync(string source)
    {
        ...
        return await compilation.WithAnalyzers(analyzers: [new SecurityUsageAnalyzer()]).GetAnalyzerDiagnosticsAsync();
    }
}
